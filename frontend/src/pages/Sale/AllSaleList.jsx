...
export default function AllSaleList() {
  ...
  //   const createdDate = new Date(sale?.created_at).toLocaleDateString("en-IN", {
  //       day: "numeric",
  //       month: "numeric",
  //       year: "numeric",
  //     });
  //   return (
  //     (sale?.Party_Name &&
  //       sale.Party_Name.toLowerCase().startsWith(lowerSearch)) ||
  //     (sale?.Payment_Type &&
  //       sale.Payment_Type.toLowerCase().startsWith(lowerSearch)) ||
  //     (sale?.created_at &&
  //       createdDate.startsWith(lowerSearch)) ||
  //     (sale?.Total_Amount &&
  //       sale.Total_Amount.toString().startsWith(lowerSearch)) ||
  //     (sale?.Balance_Due &&
  //       sale.Balance_Due.toString().startsWith(lowerSearch))
  //   );
  // });

  //    useEffect(() => {
  //         if (sales && sales?.sales?.length > 0) {
  //             setSelectedSales(sales?.sales[0]);
  //         }
  //     }, [sales]);

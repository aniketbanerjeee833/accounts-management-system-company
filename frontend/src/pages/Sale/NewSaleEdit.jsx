...
console.log(itemsValues,"itemsValues");
 ...
 {/* <div className="input-field col s6  relative">
        ...
        setPartySearch(exists.Party_Name);
        setValue("Party_Name", exists.Party_Name, { shouldValidate: true, shouldDirty: true });

        // ✅ Check GSTIN (must be present)
        if (!exists.GSTIN || exists.GSTIN.trim() === "") {
         
          setValue("GSTIN", "", { shouldValidate: true });
        } else {
          setValue("GSTIN", exists.GSTIN, { shouldValidate: true, shouldDirty: true });
        }
  ...
  {errors?.Party_Name && (
    <p className="text-red-500 text-xs mt-1">{errors?.Party_Name?.message}</p>
                ...
                <div className="input-field col s6 mt-6">
                    <span className="active">
                    GSTIN
...
<div className="flex  justify-between gap-4 w-full"> 
  ...
  e.target.value = val;
        // recalc balance
        const totalReceived = parseFloat(e.target.value || 0);
        setValue("Total_Received", e.target.value);

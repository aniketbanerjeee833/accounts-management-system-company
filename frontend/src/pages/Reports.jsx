...
//    }, [leadsForToday, leadsForTodayError, leadsLoading, selectedDate]);
 ...
 const renderCalendar = () => {
  ...
  const newSalesEachDay = totalNewSalesByDate?.reduce((acc, item) => {
    acc[item.date] = item.total_new_sales;
    return acc;
  }, {}) || {};
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    days.push(
      ...
      //         flex items-center">
      //      <span className=" text-[10px] text-green-700 font-medium"> Sales:{totalSales}</span>
      //     </div>
      //   )}
      ...
      //       <span className=" text-[10px] text-red-700 font-medium">Purch:{totalPurchases}</span>
      ...
      //       {/* {totalNewSales > 9 ? "9+" : totalNewSales} */}
      //     </div>
      //   )}
      //   </div>
      // </div>
...
>
  ...
  //    let salesEachDay = {};
  //    let newSalesEachDay={};
  //    let purchasesEachDay={};
   
  //   if(totalSalesByDate?.length > 0){
  //     salesEachDay = totalSalesByDate?.reduce((acc, item) => {
  //       acc[item.date] = item.count;
  //       return acc;
  //     }, {});
  //   }
  //   if(totalNewSalesByDate?.length > 0){
  //     newSalesEachDay = totalNewSalesByDate?.reduce((acc, item) => {
  //       acc[item.date] = item.count;
  //       return acc;
  //     }, {});
  //   }
  //   if(totalPurchasesByDate?.length > 0){
  //     purchasesEachDay = totalPurchasesByDate?.reduce((acc, item) => {
  //       acc[item.date] = item.count;
  //       return acc;
  //     }, {});
  //   }
  //   console.log(salesEachDay,newSalesEachDay,purchasesEachDay);
  ...
  //      const dateStr = formatDate(year, month, day);
  //     //  const leadCount = salesNewSalesPurchasesEachDay[dateStr] || 0;
  //     const totalSalesEachDay = salesEachDay[dateStr] || 0;
  //     const totalNewSalesEachDay = newSalesEachDay[dateStr] || 0;
  //     const totalPurchasesEachDay = purchasesEachDay[dateStr] || 0;
  //     console.log(totalSalesEachDay,totalNewSalesEachDay,totalPurchasesEachDay);
  //      const isSelected = selectedDate === dateStr;
  ...
  //          className={`h-20 border border-gray-200 p-1 cursor-pointer relative transition-colors
  //            ${isSelected ? 'bg-blue-100 border-blue-300' : isToday ? 'bg-green-100 border-green-300' : 'bg-white hover:bg-gray-50'}
  //          `}
  //        >
  //          <div className={`text-sm font-medium ${isToday ? 'text-blue-800' : 'text-gray-800'}`}>
  //            {day}
  //          </div>
  //          {totalNewSalesEachDay > 0 && (
  ...
  //              {totalNewSalesEachDay > 9 ? '9+' : totalNewSalesEachDay}
  //            </div>
  //          )}
  //        </div>
  //      );
...
//    }
   ...
   //const renderSelectedLeads = () => {
     //if (!selectedDate) {
       //return (
         //<div className="text-center text-gray-500 py-8">
           //Click on a date to view follow-up leads
         //</div>
       //);
     //}
    ...
    //  if (!selectedLeads || selectedLeads.length === 0 || leadsForTodayError) {
    //    return (
    //      <div className="text-center text-gray-500 py-8">
    //        No follow-up leads scheduled for {selectedDate}
    //      </div>
    //    );
    //  }
 
    //  return selectedLeads.slice(0,3).map((lead) => (
    //    <div
    //      key={lead.id}
    //      className="p-4 bg-green-50 border-l-4 border-green-500 rounded-r-md flex justify-between items-center mb-3"
    //    >
    //      <div>
    //        <p className="font-medium text-gray-800">{`Lead  Name:${lead?.Lead_Name || ""}`}</p>
    //        <p className="font-medium text-gray-800">{`Lead Phone Number:${lead?.Lead_Phone_Number || ""}`}</p>
               
    //        {lead.transferred_by!==null && lead.staffId!==lead?.userId &&<p className="font-medium text-gray-800">
    //         {`Transferred By:${lead?.transferred_by}`}
    ...
    //        state={{ from: "admin-scheduled-leads" }} 
